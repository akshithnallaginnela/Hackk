...
export default function App() {
  ...
  const handleIssue = async (e) => {
    ...
    try {
      ...
      const pushedLive = wsStatus === "connected" && wsRef.current;
      ...
      // Add to government issuance logs for analytics
      const historyLog = JSON.parse(localStorage.getItem("govIssuanceHistory") || "[]");
      historyLog.unshift({
        name,
        birthYear: parseInt(birthYear),
        aadhaarNumber,
      ...
      localStorage.setItem("govIssuanceHistory", JSON.stringify(historyLog));
    ...
    <>
      ...
      <div className="app-content" style={{ maxWidth: activePage === "dashboard" ? "800px" : "600px" }}>
        ...
        <nav className="gov-nav">
          <div className="gov-nav-brand" onClick={() => setActivePage("dashboard")} style={{ cursor: "pointer" }}>
            <span style={{ marginRight: 6 }}>🏛️</span> UIDAI Issuer
          </div>
          <div className="gov-nav-links">
            <button
              className={`gov-nav-link ${activePage === "dashboard" ? "active" : ""}`}
